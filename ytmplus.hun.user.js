...
// @icon         https://cdn.discordapp.com/attachments/881793296308002836/1075425076167577680/image.png
// @require      https://openuserjs.org/src/libs/sizzle/GM_config.js
// @grant        GM_getValue
// @grant        GM_setValue
// @author (2nd) Balint2201#9221 or Foxuser#9447
// ==/UserScript==
...
(function() {
    ...
    const GM_config = new GM_configStruct({
        ...
        'fields': {
            'noAfk': {
                'label': 'Soha ne legyen AFK',
                'section': 'Segédprogramok',
                'type': 'checkbox',
                'default': true
            },
            'noPromo': {
                'label': 'YouTube Premium Promóciók Kikapcsolása',
                'type': 'checkbox',
                'default': true
            },
            'skipDisliked': {
                'label': 'Dislike-olt zenék kihagyása (Beta)',
                'type': 'checkbox',
                'default': false
            },
            'padding': {
                'label': 'Kitöltés módosítása (Frissítés kell a változásokhoz)',
                'type': 'checkbox',
                'default': true
            },
            'extraButtons': {
                'label': 'Extra Playback Gombok',
                'type': 'checkbox',
                'default': true
            },
            'section0': {
            ...
            'bg': {
                'label': 'Háttér megváltoztatása',
                'section': 'Háttér beállításai',
                'type': 'checkbox',
                'default': true
            },
            'bgColor': {
            ...
            'bgEnableGradient': {
                'label': 'Színátmenet engedélyezése',
                'type': 'checkbox',
                'default': true
            },
            'bgGradient': {
                'label': 'Háttér színátmenet színe',
                'type': 'color',
                'default': '#0000AA'
            },
            'section1': {
            ...
            'clock': {
                ...
                'section': 'Óra',
                'type': 'select',
                'options': ['Eredeti', 'Gomb Törlése', 'Digitális Óra'],
                'default': 'Digitális Óra'
            },
            'clockColor': {
                'label': 'Óra színe',
                'type': 'color',
            ...
            'visualizerPlace': {
                'label': 'Zene vizualizáló helye',
                'section': 'Zene vizualizáló',
                'type': 'select',
                'options': ['Nincs', 'Navigációs sáv', 'Album kép'],
                'default': 'Album kép'
            },
            'visualizerStartsFrom': {
                'label': 'A megjelenítő innen induljon',
                'type': 'select',
                'options': ['Balról', 'Középről', 'Jobbról', 'Szélektől'],
                'default': 'Középről'
            },
            'visualizerColor': {
                'label': 'Vizualizáló színe',
                'type': 'color',
                'default': '#8C008C'
            },
            'visualizerFade': {
                'label': 'A Bar Fade engedélyezése',
                'type': 'checkbox',
                'default': false
            },
            'visualizerFft': {
                'label': 'Vonalak Mennyisége',
                'type': 'select',
                'options': ['32', '64', '128', '256', '512', '1024', '2048', '4096', '8192', '16384'],
                'default': '1024',
            },
            'attention0': {
                'label': 'A magas értékek befolyásolják a teljesítményt, és<br>megszakíthatják a kör megjelenítését.',
                'type': 'hidden'
            },
            'section3': {
                'type': 'hidden',
            ...
            'visualizerRotate': {
                'label': 'Kör Forgatása',
                'section': 'Kör vizualizáló',
                'type': 'select',
                'options': ['Nincs', 'Be', 'Reaktív', 'Reaktív (Basszusra)'],
                'default': 'Nincs'
            },
            'visualizerRotateDirection': {
                'label': 'Forgásirány',
                'type': 'select',
                'options': ['Óramutató járásával megegyező', 'Óramutató járásával ellentétes'],
                'default': 'Óramutató járásával megegyező'
            },
            'visualizerMove': {
                'label': 'Vonalak mozgása',
                'type': 'select',
                'options': ['Belül', 'Kívül', 'Mindkét oldal'],
                'default': 'Mindkét oldal'
            },
            ...
            'attention1': {
                'label': 'Az itteni változtatások hibákat okozhatnak!',
                'section': 'A vizualizáló speciális beállításai',
                'type': 'hidden'
            },
            ...
            'visualizerBassBounceSensitivityStart': {
                'label': 'Bass Bounce érzékenységi teszt indítása',
                'type': 'float',
                'min': 0,
                'max': 1,
                'default': 0
            ...
            'visualizerBassBounceSensitivityEnd': {
                'label': 'Bass Bounce érzékenységi teszt befejezése',
                'type': 'float',
                'min': 0.00001,
                'max': 1,
                'default': 0.004
        ...
        'events': {
            'open': (doc, win, frame) => { // open function is mostly customizing settings UI
                ...
                <svg viewBox="0 0 613 99">
                    <g style="overflow:hidden; text-anchor: middle;">
                        ...
                        <a href="https://github.com/Balint2201/YTMPlus_HUN#ytmplus_hun" target="_blank"><text style="filter: url(#glow);" x="50%" y="50%" dy=".35em" text-anchor="middle">YTMPlus HUN</text></a>
                    </g>
                </svg>`;
            ...
            'save': () => {
                ...
                if(GM_config.get('visualizerPlace') != 'Nincs') {
    ...
    window.addEventListener('load', () => {
        ...
        if(GM_config.get('visualizerPlace') != 'Nincs') getVideo();
    ...
    function clockEnable(mode) {
        ...
        a.background = mode != 'Digitális Óra' ? '' : `linear-gradient(to right, ${GM_config.get('clockColor')} 0%, ${GM_config.get('clockGradient') == true ? GM_config.get('clockGradientColor') : GM_config.get('clockColor')} 50%, ${GM_config.get('clockColor')} 100%`;
        a.backgroundSize = mode != 'Digitális Óra' ? '' : '200% auto';
        a.backgroundClip = mode != 'Digitális Óra' ? '' : 'text';
        a.textFillColor = mode != 'Digitális Óra' ? '' : 'transparent';
        a.webkitBackgroundClip = mode != 'Digitális Óra' ? '' : 'text';
        a.webkitTextFillColor = mode != 'Digitális Óra' ? '' : 'transparent';
        a.fontSize = mode != 'Digitális Óra' ? '20px' : '50px';
        a.animation = mode != 'Digitális Óra' ? '' : 'effect 2s linear infinite normal';
    }
    ...
    function startVisualizer() {
        ...
        switch(visualizer.place) {
            case 'Navigációs sáv': default: canvas = document.getElementById('visualizerNavbarCanvas'); break;
            case 'Album kép': canvas = document.getElementById('visualizerAlbumCoverCanvas'); break;
        }
        ...
        function visualizerResizeFix() {
            switch(visualizer.place) {
                case 'Navigációs sáv': default:
                    ...
                    if(['Középről', 'Szélektől'].includes(visualizer.startsFrom)) barTotal = WIDTH / 2 / visualizer.bufferLength;
        ...
        function visualizerNavbar() {
            switch(visualizer.startsFrom) {
                case 'Középről': xPosOffset = barWidth / 2; break;
                case 'Szélektől': xPosOffset = barSpace / 2; break;
            ...
            if(visualizer.startsFrom == 'Középről') xPosOffset = WIDTH / 2 + barWidth / 2 + barSpace;
            else if(visualizer.startsFrom == 'Szélektől') xPosOffset = barWidth + (barSpace / 2);
            else return;

            for(let i = 0; i < visualizer.bufferLength; i++) {
                barHeight = visualizer.dataArray[i] * (HEIGHT / 255);
                if(visualizer.fade) ctx.fillStyle = visualizer.color + (visualizer.dataArray[i] < 128 ? (visualizer.dataArray[i] * 2).toString(16) : 'FF');
                if(visualizer.startsFrom == 'Középről') {
        ...
        function visualizerCircle() { // Bitwise truncation (~~number) is used here instead of Math.floor() to squish out more performance.
            if(visualizer.startsFrom == 'Balról' || visualizer.startsFrom == 'Jobbról') circleSize = 2; // 2PI = full, 1PI = half;
            else circleSize = 1;

            if(visualizer.bassBounce.enabled || visualizer.rotate == 'Reaktív (Basszusra)') {
            ...
            switch(visualizer.rotate) {
                case 'Be':
                    if(visualizer.rotateDirection == 'Óramutató járásával megegyező') rotationValue += 0.005;
                    else rotationValue -= 0.005;
                    break;
                case 'Reaktív':
                    if(visualizer.rotateDirection == 'Óramutató járásával megegyező') rotationValue += Math.pow(visualizer.dataArray.reduce((a, b) => a + b, 0) / visualizer.bufferLength / 10000 + 1, 2) - 1;
                    else rotationValue -= Math.pow(visualizer.dataArray.reduce((a, b) => a + b, 0) / visualizer.dataArray.length / 10000 + 1, 2) - 1;
                    break;
                case 'Reaktív (Basszusra)':
                    if(visualizer.rotateDirection == 'Óramutató járásával megegyező') rotationValue += Math.pow(bassSmoothRadius * 2 / 10000 + 1, 2) - 1;
            ...
            function drawArcs(backwards) {
                ...
                for(let i = 0; i < visualizer.dataArray.length; ++i) {
                    ...
                    if(visualizer.move == 'Kívül' || visualizer.move == 'Mindkét oldal') outerRadius = radius + barHeight;
                    else outerRadius = radius;
                    if(visualizer.move == 'Belül' || visualizer.move == 'Mindkét oldal') innerRadius = radius - barHeight;
            ...
            if(visualizer.startsFrom == 'Jobbról' ||
                visualizer.startsFrom == 'Középről' ||
                visualizer.startsFrom == 'Szélektől') drawArcs();

            if(visualizer.startsFrom == 'Balról' ||
                visualizer.startsFrom == 'Középről' ||
                visualizer.startsFrom == 'Szélektől') drawArcs(true);
        }
    }
})();
